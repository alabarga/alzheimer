...
meth450_data_original <-lumiMethyR(filename=MethylDataFileName, sampleDescriptions=sampleDescriptions, lib= 'FDb.InfiniumMethylation.hg19' )
meth450_data_original_2 <-methylumiR(filename=MethylDataFileName, sampleDescriptions=sampleDescriptions )
...
plotSampleRelation(meth450_data_original, method='mds', cv.Th=0, color=sampleDescriptions$Lote)

density(meth450_data_original,col=sampleDescriptions$Lote)

plotColorBias1D(meth450_data_original, channel='sum')
plotColorBias1D(meth450_data_original, channel='methy')
plotColorBias1D(meth450_data_original, channel='unmethy')

boxplot(estimateIntensity(meth450_data_original),col=as.numeric(sampleDescriptions$Lote) + 1)

################ REMOVE BAD SAMPLES ############################################

meth450_data <- meth450_data_original_2
...
meth450_norm_A <- normalize450(meth450_data[,which(samps$Lote=='A')], method=normalization)
meth450_norm_B <- normalize450(meth450_data[,which(samps$Lote=='B')], method=normalization)

explore450(meth450_norm, group="status")
...
M_A <- exprs(meth450_norm_A)
M_B <- exprs(meth450_norm_B)
...
M <- removeBatchEffect(M,batch=samps[colnames(M),]$Lote, design=design)
b <- 2^M / (2^M + 1)
...
nrow(dmrs)
View(head(dmrs[order(-abs(dmrs$maxbetafc)),],50))
DMR.plot(dmrcoutput=dmrcoutput, dmr=which(rownames(dmrcoutput$results)==204), betas=b, 
         phen.col=c("red","blue")[clases], 
...
write.csv(dmrs, file=paste("./data", projectName,"results","dmrs_30042015.csv",sep="/"))
...
top <- topTest(res, diff=0.05, pval = 1)
top <- topTest(res, diff=0.05, pval= 0.01,  pval.var= "P.Value", diff.var="Beta.Difference")
...
write.csv(table_selected, file=paste("./data", projectName,"results","table_total_30042015.csv",sep="/"))
...
samps <- samps[colnames(M),]
plotMDS(b[selected,], labels=samps$sampleID, col=as.integer(clases))
b_heatmap.3(b, selected, clases)
volcano450(res,  adjust.p = 0.01, diff.thresh=0.05, pval.var="P.Value", diff.var="Beta.Difference")
beta_FC(top, pval.var= "P.Value", adjust.p = 0.001)
